...
class MessagingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create messaging' do
    assert_difference('Messaging.count') do
      post messagings_url,
           params: { messaging: { deleted: @messaging.deleted, message: @messaging.message, read: @messaging.read,
                                  receiver: @messaging.receiver, replying_to: @messaging.replying_to, sender: @messaging.sender } }
    end

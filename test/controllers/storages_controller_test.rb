...
class StoragesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create storage' do
    assert_difference('Storage.count') do
      post storages_url, params: { storage: { company_branch_id: @storage.company_branch_id } }
    end
  ...
  test 'should update storage' do
    patch storage_url(@storage), params: { storage: { company_branch_id: @storage.company_branch_id } }
